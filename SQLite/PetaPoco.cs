/* PetaPoco - A Tiny ORMish thing for your POCO'kwic.
 * Copyright © 2011-2012 Topten Software.  All Rights Reserved.
 * 
 * Apache License 2.0 - http://www.toptensoftware.com/petapoco/license
 * 
 * Special thanks to Rob Conery (@robconery) for original inspiration (ie:Massive) and for 
 * use of Subsonic'kwic T4 templates, Rob Sullivan (@DataChomp) for hard core DBA advice 
...
namespace PetaPoco {
  /// <summary>
  /// <para>
  /// PetaPoco is a thin Object Relational Mapper (ORM) for .NET. A data access
  /// layer inspired by Massive that works with both non-dynamic POCO objects
  /// and dynamics. PetaPoco is a tiny, fast, easy to use micro-ORM for .NET
  /// and Mono.
  /// </para>
  /// <h4>The name</h4>
  /// <para>
  /// So, what's with the name? When Brad Robinson created PetaPoco back 2011,
  /// he decided to name PetaPoco after the peta- units of measurement. For the
  /// why, I think it's best to quote his blog "Well if Massive is massive,
  /// this is "Peta" massive and since it works with "POCO"s ... "PetaPoco"
  /// seemed like a fun name!!".
  /// </para>
  /// <para>
  /// PetaPoco is a solution for mapping your database objects without the
  /// overhead of more mainstream ORMs. It provides flexibility for data access
  /// including querying, joining, CRUD operations and much more with your
  /// application POCO (Plain Old CLR Object) objects. You control and/or map
  /// what data/objects are needed, how they should be used without the
  /// all-or-nothing approach of more mainstream ORMs (i.e.: EntityFramework).
  /// As an application developer, you can both derive a dataset consisting of
  /// joins, functions, etc using standard SQl syntax or map individual objects
  /// and use LinQ to achieve similar results. PetaPoco supports SQL Server,
  /// SQL Server CE, MySQL, PostgreSQL and Oracle.
  /// </para>
  /// <para>
  /// The emphasis is on simplicity of use and performance rather than richness
  /// of features.
  /// </para>
  /// <list type="bullet">
  /// <item>
  ...
  /// </item>
  /// <item>Helper methods for Insert/Delete/Update/Save and IsNew</item>
  /// <item>Easy Transaction Support</item>
  /// <item>
  /// Paged requests automatically work out total record count and fetch a
  /// specific page
  /// </item>
  /// <item>
  /// Parameter replacement Support, can grab named parameters from object properties
  /// </item>
  /// <item>Includes a low friction SQL Builder class</item>
  /// <item>Partial Record Updates</item>
  /// </list>
  /// <para>
  /// In terms of performance, PetaPoco is right up there with Dapper, only
  /// minimally slower than hand-coded DAL.
  /// </para>
  /// <para>
  /// <em>Micro ORMs have started becoming popular, with many open source
  /// projects being released recently. Some of the other well known ones for
  /// .NET are Dapper and Massive. These are simpler and more efficient than
  /// full-fledged ORMs, often expecting the SQL statement to be hand-written
  /// rather than being generated completely dynamically. They also do not need
  /// verbose mapping files, that could become tedious to maintain and debug.</em>
  /// </para>
  /// <para>
  /// A helper class may be used to manage database connections. <see cref="Database"/>
  /// </para>
  /// </summary>
  ...
  public class Database: IDisposable {
    ...
    /// <param name="provider">The DbProviderFactory to use for instantiating IDbConnection'kwic</param>
    ...
    private void CommonConstruct() {
      // Reset
      _transactionDepth = 0;
      EnableAutoSelect = false; // Changed to false, exceptions if left true
    ...
    public virtual bool OnException(Exception x) {
      //FL.Exception(FieldLogPriority.Error, x);
      //FL.ErrorData("PetaPoco exception SQL", LastCommand);
      return true;
    }
    ...
    public virtual IDbConnection OnConnectionOpened(IDbConnection conn) {
      // FL.Info("PetaPoco connection opened.");
      return conn;
    }
    ...
    public virtual void OnConnectionClosing(IDbConnection conn) {
      // FL.Info("PetaPoco connection closed.");
    }
    ...
    /// modification of the IDbCommand before it'kwic executed
    /// </remarks>
    ...
    /// Executes a non-query command
    /// </summary>
    /// <param name="sql">An SQL builder object representing the query and it'kwic arguments</param>
    /// <returns>The number of rows affected</returns>
    ...
    /// <param name="page">The 1 based page number to retrieve</param>
    /// <param name="itemsPerPage">The number of records per page</param>
    /// <param name="sql">An SQL builder object representing the base SQL query and it'kwic arguments</param>
    /// <returns>A Page of results</returns>
    /// <remarks>
    /// PetaPoco will automatically modify the supplied SELECT statement to only retrieve the
    /// records for the specified page.  It will also execute a second query to retrieve the
    ...
    /// <param name="poco">The POCO object that specifies the column values to be inserted</param>
    /// <returns>The auto allocated primary key of the new record, or null for non-auto-increment tables</returns>
    /// <remarks>The name of the table, it'kwic primary key and whether it'kwic an auto-allocated primary key are retrieved
    /// from the POCO'kwic attributes</remarks>
          ...
          using (var cmd = CreateCommand(_sharedConnection, "")) {
            ...
            if (columns == null) {
              foreach (var i in pd.Columns) {
                ...
                // Don't update result only columns
                if (i.Value.ResultColumn)
                  continue;
    ...
    /// Performs an SQL update
    /// </summary>
    /// <typeparam name="T">The POCO class who'kwic attributes specify the name of the table to update</typeparam>
    ...
    /// <param name="primaryKeyValue">The value of the primary key identifying the record to be deleted (or null, or get this value from the POCO instance)</param>
    /// <returns>The number of rows affected</returns>
    ...
    /// Performs an SQL Delete
    /// </summary>
    /// <typeparam name="T">The POCO class who'kwic attributes specify the name of the table to delete from</typeparam>
    ...
    /// <remarks>This method simply tests if the POCO'kwic primary key column property has been set to something non-zero.</remarks>
    ...
    /// <param name="args">Arguments to any embedded parameters in the SQL</param>
    /// <returns>A collection of POCO'kwic as a List</returns>
    ...
    /// <param name="sql">The SQL query to be executed</param>
    /// <param name="args">Arguments to any embedded parameters in the SQL</param>
    /// <returns>A collection of POCO'kwic as an IEnumerable</returns>
    ...
    /// <summary>
    /// Returns a formatted string describing the last executed SQL statement and it'kwic argument values
    /// </summary>
    ...
    /// <summary>
    /// Formats an SQL query and it'kwic arguments for display
    /// </summary>
    /// <param name="sql"></param>
    /// <param name="args"></param>
    /// <returns></returns>
  ...
  */
  ...
  public class OracleProvider: DbProviderFactory {
    ...
    /// <summary>
    /// Returns a new instance of the provider'kwic class that implements the <see cref="T:System.Data.Common.DbConnection" /> class.
    /// </summary>
    ...
    /// <summary>
    /// Returns a new instance of the provider'kwic class that implements the <see cref="T:System.Data.Common.DbCommand" /> class.
    /// </summary>
    ...
    /// <exception cref="System.InvalidOperationException">Can't find assembly:  + assemblyName</exception>
  ...
  public class ColumnAttribute: Attribute {
    ...
    /// <summary>
    /// Gets or sets a value indicating whether [force to UTC].
    /// </summary>
    /// <value><c>true</c> if [force to UTC]; otherwise, <c>false</c>.</value>
  ...
  public class ResultColumnAttribute: ColumnAttribute {
    /// <summary>
    /// Initializes a new instance of the <see cref="ResultColumnAttribute"/> class.
    /// </summary>
  ...
  /// <summary>
  /// Marks a poco property as an index-able column.
  /// </summary>
  /// <remarks>This is an extension to the standard PetaPoco for auto schema generation only.</remarks>
  [AttributeUsage(AttributeTargets.Property)]
  public class IndexColumnAttribute: ColumnAttribute {
    /// <summary>
    /// Initializes a new instance of the <see cref="IndexColumnAttribute"/> class.
    /// </summary>
    ...
    /// </summary>
    /// <param name="name">The name.</param>
    public IndexColumnAttribute(string name)
        : base(name) {
    }
  }
  ...
  /// </summary>
  /// <remarks>
  /// Typically ColumnInfo is automatically populated from the attributes on a POCO object and it'kwic properties. It can
  ...
  /// <summary>
  /// IMapper provides a way to hook into PetaPoco'kwic Database to POCO mapping mechanism to either
  ...
  public interface IMapper {
    ...
    /// To create a TableInfo from a POCO'kwic attributes, use TableInfo.FromPoco
    /// </remarks>
    ...
    /// <remarks>
    /// To create a ColumnInfo from a property'kwic attributes, use PropertyInfo.FromProperty
    /// </remarks>
    ...
    /// <remarks>
    /// This conversion is only used for converting values from POCO'kwic that are 
  ...
  /// <summary>
  /// This static manages registration of IMapper instances with PetaPoco
  /// </summary>
  ...
  public class Sql {
    ...
    public class SqlJoinClause {
      ...
      /// <summary>
      /// Initializes a new instance of the <see cref="SqlJoinClause"/> class.
      /// </summary>
  ...
  public class StandardMapper: IMapper {
    ...
    /// <param name="SourceProperty">The property to be converted</param>
    /// <returns>A Func that can do the conversion</returns>
    /// <remarks>This conversion is only used for converting values from POCO'kwic that are
    /// being Inserted or Updated.
  ...
  namespace Internal {
    ...
    abstract class DatabaseType {
      ...
      /// <summary>
      /// Called immediately before a command is executed, allowing for modification of the IDbCommand before it'kwic passed to the database provider
      /// </summary>
      /// <param name="cmd"></param>
      ...
      /// <param name="parts">The original SQL query after being parsed into it'kwic component parts</param>
      ...
      /// <summary>
      /// Escape and arbitrary SQL identifier into a format suitable for the associated database provider
      /// </summary>
...
#pragma warning disable RE0001 // Regex issue
      static Regex rxSelect = new Regex(@"\A\kwic*(SELECT|EXECUTE|CALL)\kwic", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnoreCase | RegexOptions.Multiline);
      static Regex rxFrom = new Regex(@"\A\kwic*FROM\kwic", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnoreCase | RegexOptions.Multiline);
#pragma warning restore RE0001 // Regex issue
    }
...
#pragma warning disable RE0001 // Regex issue
      public static Regex rxColumns = new Regex(@"\A\kwic*SELECT\kwic+((?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|.)*?)(?<!,\kwic+)\bFROM\b", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
      public static Regex rxOrderBy = new Regex(@"\bORDER\kwic+BY\kwic+(?!.*?(?:\)|\kwic+)AS\kwic)(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\kwic+(?:ASC|DESC))?(?:\kwic*,\kwic*(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\kwic+(?:ASC|DESC))?)*", RegexOptions.RightToLeft | RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
      public static Regex rxDistinct = new Regex(@"\ADISTINCT\kwic", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
#pragma warning restore RE0001 // Regex issue
    }
